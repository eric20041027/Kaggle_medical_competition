"""
BioLinkBERT-large K-Fold Fine-tuning（Google Colab 版）
=====================================================
【執行步驟】
  ...
  # Cell 3：訓練
  !python colab_biolinkbert_kfold.py

  # Cell 4：下載結果
  from google.colab import files
  files.download('/content/drive/MyDrive/kaggle_biolinkbert/test_probs_biolinkbert.npy')
  files.download('/content/drive/MyDrive/kaggle_biolinkbert/oof_probs_biolinkbert.npy')

【斷線後重跑】
  from google.colab import drive
  drive.mount('/content/drive')
  %cd /content/Kaggle_medical_competition
  !python colab_biolinkbert_kfold.py  # 自動跳過已完成的 fold
"""
...
_DRIVE_DIR = "/content/drive/MyDrive/kaggle_biolinkbert"
_LOCAL_DIR = "outputs/colab_biolinkbert"
...
# ── 斷點續訓：載入上次進度 ────────────────────────────────────────
_oof_path  = os.path.join(OUTPUT_DIR, "oof_probs_biolinkbert.npy")
_test_path = os.path.join(OUTPUT_DIR, "test_probs_biolinkbert.npy")
...
for fold, (train_idx, val_idx) in enumerate(skf.split(all_texts, all_labels)):
    ...
    best_f1    = 0.0
    no_improve = 0
    best_path  = os.path.join(OUTPUT_DIR, f"biolinkbert_fold{fold+1}.pt")
...
submission.to_csv(os.path.join(OUTPUT_DIR, "submission_biolinkbert.csv"), index=False)

np.save(_oof_path,  oof_probs)
np.save(_test_path, test_probs)

print(f"\n提交檔 → {OUTPUT_DIR}/submission_biolinkbert.csv")
print(f"預測分布:\n{pd.Series(test_pred_submit).value_counts().sort_index()}")
print(f"\n完成！OOF Macro F1 = {oof_f1:.4f}")
print(f"\n[後續] 下載 test_probs_biolinkbert.npy 與 oof_probs_biolinkbert.npy 供 ensemble 使用")
