"""
BiomedBERT-large K-Fold Fine-tuning（Google Colab 版）
=====================================================
【執行步驟】
  ...
  # Cell 3：訓練
  !python colab_biomedbert_kfold.py

  # Cell 4：下載結果
  from google.colab import files
  files.download('/content/drive/MyDrive/kaggle_biomedbert/test_probs_biomedbert.npy')
  files.download('/content/drive/MyDrive/kaggle_biomedbert/oof_probs_biomedbert.npy')

【斷線後重跑】
  from google.colab import drive
  drive.mount('/content/drive')
  %cd /content/Kaggle_medical_competition
  !python colab_biomedbert_kfold.py  # 自動跳過已完成的 fold
"""
...
_DRIVE_DIR = "/content/drive/MyDrive/kaggle_biomedbert"
_LOCAL_DIR = "outputs/colab_biomedbert"
...
# ── 斷點續訓：載入上次進度 ────────────────────────────────────────
_oof_path  = os.path.join(OUTPUT_DIR, "oof_probs_biomedbert.npy")
_test_path = os.path.join(OUTPUT_DIR, "test_probs_biomedbert.npy")
...
# ── 最終結果 ──────────────────────────────────────────────────────
print(f"\n{'='*60}\n  BiomedBERT-large K-Fold 訓練完成（Colab）\n{'='*60}")
...
submission.to_csv(os.path.join(OUTPUT_DIR, "submission_biomedbert.csv"), index=False)

np.save(_oof_path,  oof_probs)
np.save(_test_path, test_probs)

print(f"\n提交檔 → {OUTPUT_DIR}/submission_biomedbert.csv")
print(f"預測分布:\n{pd.Series(test_pred_submit).value_counts().sort_index()}")
print(f"\n完成！OOF Macro F1 = {oof_f1:.4f}")
print(f"\n[後續] 下載 test_probs_biomedbert.npy 與 oof_probs_biomedbert.npy 供 ensemble 使用")
