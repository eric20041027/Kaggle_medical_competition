"""
DeBERTa-v3-large K-Fold Fine-tuning
====================================
目的：與 BiomedBERT-large 做 ensemble（不同架構 → 互補錯誤）

與 BiomedBERT-large v2 的差異：
  - MODEL_NAME: deberta-v3-large（通用強模型，disentangled attention）
  - LR: 5e-6（deberta 學習率宜更小）
  - WARMUP_RATIO: 0.10（deberta 從 pretrained 起步較穩）
  - GRAD_ACCUM: 16（deberta 記憶體佔用略高，縮小 per-step batch）
  - 輸出: test_probs_deberta.npy（與 biomedbert 軟機率合併 ensemble）
"""
...
MAX_LEN      = 384
BATCH_SIZE   = 8
GRAD_ACCUM   = 8          # effective batch = BATCH_SIZE(8) * GRAD_ACCUM(8) = 64
EPOCHS       = 8
LR           = 5e-6       # deberta 比 biomedbert 用更小 LR
WARMUP_RATIO = 0.10
LABEL_SMOOTH = 0.1
N_FOLDS      = 3
PATIENCE     = 3
...
class MedicalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # DeBERTa 不需要 token_type_ids，AutoTokenizer 不會產生它
        item = {
...
print(f"\n[後續] 下載 test_probs_deberta.npy 供 ensemble 使用")
