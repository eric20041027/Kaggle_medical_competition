"""
BioLinkBERT-large + FGM 對抗性訓練（Google Colab 版）
=====================================================
FGM（Fast Gradient Method）在 Embedding 層加入擾動，提升泛化能力。
預期比普通微調提升 LB +0.005～0.01。
...
【執行步驟】
  ...
  # Cell 3：訓練
  !python colab_biolinkbert_fgm_fgm_kfold.py

  # Cell 4：下載結果
  from google.colab import files
  files.download('/content/drive/MyDrive/kaggle_biolinkbert_fgm_fgm/test_probs_biolinkbert_fgm.npy')
  files.download('/content/drive/MyDrive/kaggle_biolinkbert_fgm_fgm/oof_probs_biolinkbert_fgm.npy')

【斷線後重跑】
  from google.colab import drive
  drive.mount('/content/drive')
  %cd /content/Kaggle_medical_competition
  !python colab_biolinkbert_fgm_fgm_kfold.py  # 自動跳過已完成的 fold
"""
...
_DRIVE_DIR = "/content/drive/MyDrive/kaggle_biolinkbert_fgm_fgm"
_LOCAL_DIR = "outputs/colab_biolinkbert_fgm"
...
USE_AMP  = torch.cuda.is_available() and not USE_BF16  # FP16 AMP（T4 fallback）
...
FGM_EPSILON  = 1.0   # embedding 擾動幅度
MAX_LEN      = 384
...
print(f"BATCH_SIZE={BATCH_SIZE}  GRAD_ACCUM={GRAD_ACCUM}  N_FOLDS={N_FOLDS}  MAX_LEN={MAX_LEN}")
...
print(f"\n原始訓練集: {len(train_df)} 筆 ({train_df['condition'].nunique()} 唯一文本)")
...
# ── 斷點續訓：載入上次進度 ────────────────────────────────────────
_oof_path  = os.path.join(OUTPUT_DIR, "oof_probs_biolinkbert_fgm.npy")
_test_path = os.path.join(OUTPUT_DIR, "test_probs_biolinkbert_fgm.npy")
...
class FGM:
    """Fast Gradient Method: 在 word embedding 加入擾動提升泛化能力"""
    def __init__(self, model):
        self.model  = model
        self.backup = {}

    def attack(self, epsilon=1.0, emb_name="embeddings.word_embeddings"):
...
for fold, (train_idx, val_idx) in enumerate(skf.split(all_texts, all_labels)):
    ...
    best_f1    = 0.0
    no_improve = 0
    best_path  = os.path.join(OUTPUT_DIR, f"biolinkbert_fgm_fold{fold+1}.pt")
...
# ── 最終結果 ──────────────────────────────────────────────────────
print(f"\n{'='*60}\n  BioLinkBERT-large K-Fold 訓練完成（Colab）\n{'='*60}")
...
submission.to_csv(os.path.join(OUTPUT_DIR, "submission_biolinkbert_fgm.csv"), index=False)

np.save(_oof_path,  oof_probs)
np.save(_test_path, test_probs)

print(f"\n提交檔 → {OUTPUT_DIR}/submission_biolinkbert_fgm.csv")
print(f"預測分布:\n{pd.Series(test_pred_submit).value_counts().sort_index()}")
print(f"\n完成！OOF Macro F1 = {oof_f1:.4f}")
print(f"\n[後續] 下載 test_probs_biolinkbert_fgm.npy 與 oof_probs_biolinkbert_fgm.npy 供 ensemble 使用")
